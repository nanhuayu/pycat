...
from core.agent.run.control_messages import RESUME_INTERRUPTED_RUN
from core.app.services.conversation import ConversationRevisionResult
...
from gui.presenters.prompt_optimization_presenter import PromptOptimizationPresenter
from gui.presenters.streaming_message_presenter import StreamingMessagePresenter
from gui.runtime.background_job import BackgroundJob
from models.contracts.agent import RunEvent, RunStatus
from models.contracts.content import InputPreparationResult
...
class MessagePresenter:
    ...
    def __init__(self, host: MainWindow) -> None:
        ...
        self._approval_dialogs: dict[str, QMessageBox] = {}
        self._approval_allow_all_buttons: dict[str, object] = {}
    ...
    def on_approval_requested(
        ...
        existing = self._approval_dialogs.get(approval_id)
        if existing is not None:
            self._raise_dialog(existing)
            return
        try:
            active_modal = QApplication.activeModalWidget()
            if active_modal is not None and active_modal.objectName() == "tool_approval_dialog":
                active_modal = active_modal.parentWidget()
            parent = active_modal or self._host
            dialog = QMessageBox(parent)
            dialog.setObjectName("tool_approval_dialog")
            dialog.setWindowTitle("工具执行确认")
            ...
            dialog.setInformativeText(
                f"{request.tool_name}  ·  {request.category}  ·  {request.risk}"
            )
            ...
            if yes_button is not None:
                yes_button.setText("允许本次")
            if no_button is not None:
                no_button.setText("禁止")
            allow_all_button = dialog.addButton(
                "本会话全部放行",
                QMessageBox.ButtonRole.AcceptRole,
            )
            self._approval_dialogs[approval_id] = dialog
            self._approval_allow_all_buttons[approval_id] = allow_all_button
            dialog.finished.connect(
                lambda _result,
                conversation_id=conversation_id,
                approval_id=approval_id,
                dialog=dialog: self._finish_approval_dialog(
                    conversation_id, approval_id, dialog
                )
            )
            dialog.open()
            ...
            self._approval_dialogs.pop(approval_id, None)
            self._approval_allow_all_buttons.pop(approval_id, None)
            self._host.message_runtime.resolve_approval(approval_id, False)

    def on_approval_resolved(self, approval_id: str) -> None:
        dialog = self._approval_dialogs.pop(str(approval_id or ""), None)
        self._approval_allow_all_buttons.pop(str(approval_id or ""), None)
        if dialog is None:
            return
        if dialog.isVisible():
            dialog.close()
    ...
    def raise_pending_approval(self) -> bool:
        for dialog in reversed(list(self._approval_dialogs.values())):
            if dialog.isVisible():
                self._raise_dialog(dialog)
                return True
        return False

    def _finish_approval_dialog(
        self,
        conversation_id: str,
        approval_id: str,
        dialog: QMessageBox,
    ) -> None:
        if self._approval_dialogs.get(approval_id) is not dialog:
            dialog.deleteLater()
            return
        clicked = dialog.clickedButton()
        allow_all_button = self._approval_allow_all_buttons.get(approval_id)
        if clicked is allow_all_button:
            try:
                updated = self._host.conversation_presenter.update_permission_preset(
                    "allow",
                    confirm_allow=False,
                    conversation_id=conversation_id,
                )
            except Exception as exc:
                logger.warning("Failed to allow tools for conversation %s: %s", conversation_id, exc)
                updated = False
            if not updated:
                dialog.open()
                self._raise_dialog(dialog)
        ...
        self._approval_dialogs.pop(approval_id, None)
        self._approval_allow_all_buttons.pop(approval_id, None)
        self._host.message_runtime.resolve_approval(approval_id, approved)
        dialog.deleteLater()

    @staticmethod
    def _raise_dialog(dialog: QMessageBox) -> None:
        dialog.show()
        dialog.raise_()
    ...
    def send(self, content: str, attachments: list, metadata: Optional[dict[str, Any]] = None) -> None:
        host = self._host
        ...
        if not self._commit_message(host.current_conversation, provider, user_message):
    ...
    def _finish_turn_restart(
        ...
        *,
        ...
        if revised is not None and start_after:
            state = self.start_streaming(provider, conversation=revised)
            if state is None:
                self._show_revision_error(
                    conversation_id,
                    "消息已修订，但任务未能启动，请重试",
    ...
    def _show_revision_error(
        ...
        *,
        ...
        self._host.chat_view.show_header_notice(
            str(text or "会话修订失败"),
            tone="error",
    ...
    def _finish_attachment_prepare(
        ...
        try:
            ...
            host.window_state_presenter.sync_input_enabled()
        if committed:
            self.start_streaming(provider, conversation=latest_conversation)
    ...
    def _commit_message(
        ...
        *,
        ...
        previous_total_tokens = conversation.total_tokens
        previous_seq = conversation.current_seq_id()
        conversation.add_message(user_message)

        if len(conversation.messages) == 1:
            conversation.generate_title_from_first_message()

        if not host.services.conv_service.save(conversation, activity_token=activity_token):
            conversation.messages = [item for item in conversation.messages if item.id != user_message.id]
            conversation.title = previous_title
            conversation.updated_at = previous_updated_at
            conversation.total_tokens = previous_total_tokens
            conversation._seq_counter = previous_seq
            return False
        ...
        if start_runtime:
            self.start_streaming(provider, conversation=conversation)
        return True
    ...
    def start_streaming(self, provider: Provider, *, conversation: Conversation | None = None) -> None:
        return self._streaming_presenter.start_streaming(provider, conversation=conversation)
