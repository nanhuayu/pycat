...
class ConversationPresenter:
    ...
    def select(self, conversation_id: str) -> None:
        ...
        try:
            ...
            try:
                host.inspector_panel.update_runtime_state(stream_state)
    ...
    def update_show_thinking(self, enabled: bool) -> None:
        ...
        set_show_thinking = getattr(self._host.chat_view, "set_show_thinking", None)
        if callable(set_show_thinking):
            set_show_thinking(bool(enabled))
    ...
    def compact_current(self) -> None:
        ...
        try:
            ...
            host.inspector_panel.update_stats(conv)
    ...
    def _review_memory_candidate(self, candidate_id: str, *, promote: bool) -> None:
        ...
        try:
            ...
            host.inspector_panel.update_stats(conversation)
            host.statusBar().showMessage(result, 3000)
        except Exception as exc:
            logger.warning("Failed to review memory candidate %s: %s", target, exc)
            host.statusBar().showMessage(f"记忆候选操作失败：{exc}", 4000)
