...
from models.contracts.session_state.memory import MemoryCandidate, MemoryRecord
from models.contracts.session_state.todo import RECENT_COMPLETED_TODO_LIMIT, TodoDigest, TodoItem
from models.contracts.session_state.work_trace import WorkTrace
...
class SessionState:
    ...
    memory_candidates: Dict[str, MemoryCandidate] = field(default_factory=dict)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'memory_candidates': {
                k: v.to_dict() if isinstance(v, MemoryCandidate) else MemoryCandidate.from_dict(k, v).to_dict()
                for k, v in self.memory_candidates.items()
            },
            ...
            'last_memory_review_seq': self.last_memory_review_seq,
        }
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'SessionState':
        ...
        candidates_raw = data.get('memory_candidates', {})
        memory_candidates = {
            str(k): MemoryCandidate.from_dict(str(k), v)
        ...
        return cls(
            ...
            last_memory_review_seq=int(data.get('last_memory_review_seq', 0) or 0),
        )
