...
PermissionPreset = Literal["default", "ask", "deny", "allow", "custom"]
PERMISSION_PRESETS: tuple[PermissionPreset, ...] = ("default", "ask", "deny", "allow", "custom")
...
def normalize_permission_preset(value: str | None) -> PermissionPreset:
    raw = str(value or "").strip().lower()
...
def permission_config_for_preset(
    preset: str | None,
    custom: "ToolPermissionConfig | None" = None,
) -> "ToolPermissionConfig":
    """Expand a session permission preset into a concrete config.

    ``default`` uses the built-in category table, ``ask``/``deny``/``allow`` apply one
    action to every category, and ``custom`` uses the application-level rules
    maintained in the settings page.
    """
    normalized = normalize_permission_preset(preset)
