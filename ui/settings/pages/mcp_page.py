...
class McpPage(QWidget):
    ...
    def __init__(self, storage_service: StorageService | None = None, parent=None):
        ...
        for text in [
            "list_directory：列出目录结构，用于快速浏览工作区。",
            "read_file：读取文本文件内容，便于补充上下文。",
            "search_code：按关键字检索文件内容。",
            "python_exec：执行受控 Python 代码，用于小规模分析与验证。",
        ]:
            line = QLabel(text)
        ...
        hint = QLabel(
            "启用 MCP 后，应用会自动提供内置工具（list_directory、read_file、search_code、python_exec），\n"
