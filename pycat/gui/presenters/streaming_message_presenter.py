...
class StreamingMessagePresenter:
    ...
    def _is_current(self, conversation_id: str) -> bool:
        current = self._host.current_conversation
        return bool(current and current.id == conversation_id)

    def _ensure_overlay_started(self, conversation_id: str) -> None:
        """Start the streaming overlay (for the current conversation) if inactive."""
        host = self._host
        if host.chat_view.is_streaming():
            return
    ...
    def on_token(self, conversation_id: str, request_id: str, token: str) -> None:
        ...
        if self._is_current(conversation_id):
            self._ensure_overlay_started(conversation_id)
    ...
    def on_run_finished(
        ...
        host = self._host
        if not self._is_current(conversation_id):
            return
    ...
    def on_conversation_patch(
        ...
        try:
            ...
            domains = set((patch.diagnostics or {}).get("changed_domains") or [])
            if domains:
                host.services.app_coordinator.invalidate_content(target.work_dir, target.id, tuple(domains))
            if not changed_fields and not new_condensed:
                ...
                # persist or repaint.
                if domains and is_current:
                    self._refresh_inspector_state(target, domains)
                return
            if not is_current:
                return
    ...
    def _apply_runtime_message_updates(self, conversation, message: Message) -> None:
        ...
        if self._is_current(getattr(conversation, "id", "")):
            try:
