...
class ModelCatalogDialog(QDialog):
    ...
    def set_remote_models(self, profiles: list[ModelProfile]) -> None:
        ...
        self._status_message = "未找到远端模型" if not self._remote_ids else ""
        self._rebuild_list()

    def set_error(self, message: str) -> None:
        self._status_message = f"获取失败：{str(message or '未知错误')}"
        self._update_count()
    ...
    def _update_count(self) -> None:
        ...
        self.status_label.setText(self._status_message or f"已选 {selected} / {total}")
