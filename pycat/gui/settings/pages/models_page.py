...
class ProviderListItem(SettingsStatusListItem):
    ...
    def update_display(self) -> None:
        ...
        api_labels = {value: label for label, value in ProviderEditor.api_type_options()}
        self.set_status(
            self.provider.account_label if self.provider.is_builtin_account else self.provider.name,
            enabled=enabled,
            tooltip=(
                QCoreApplication.translate('ModelsPage', '{name}\n状态：{value}\n接口：{value_}\nAPI：{value__}').format(name=self.provider.name, value=QCoreApplication.translate('ModelsPage', '启用') if enabled else QCoreApplication.translate('ModelsPage', '停用'), value_=api_labels.get(self.provider.api_type, self.provider.api_type), value__=self.provider.api_base or '-')
            ),
        )
...
class ModelsPage(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        root.setSpacing(10)
        root.addWidget(build_page_header(QCoreApplication.translate('ModelsPage', '模型'), QCoreApplication.translate('ModelsPage', '管理默认模型、服务商连接和单模型能力参数。')))
        ...
        self.model_pool_combo = ModelRefCombo(
            ...
            allow_empty=False,
            empty_label=QCoreApplication.translate('ModelsPage', '选择模型'),
        )
        ...
        self.btn_add_provider = provider_actions.add_icon_action(
            QCoreApplication.translate('ModelsPage', '添加 API 服务商'), Icons.get(Icons.PLUS), self._add_provider,
        )
        self.btn_toggle_enabled = provider_actions.add_icon_action(
            QCoreApplication.translate('ModelsPage', '停用服务商'), Icons.get(Icons.PAUSE), self._toggle_provider_enabled,
        )
        provider_actions.add_stretch()
        self.btn_move_up = provider_actions.add_icon_action(
            QCoreApplication.translate('ModelsPage', '上移服务商'), Icons.get(Icons.ARROW_UP), lambda: self._move_provider(-1),
        )
        self.btn_move_down = provider_actions.add_icon_action(
            QCoreApplication.translate('ModelsPage', '下移服务商'), Icons.get(Icons.ARROW_DOWN), lambda: self._move_provider(1),
        )
        self.btn_delete_provider = provider_actions.add_icon_action(
            QCoreApplication.translate('ModelsPage', '删除服务商'),
        ...
        self.empty_state = SettingsEmptyState(self.tr("尚未添加模型服务"),
            self.tr("点击上方加号添加服务商，然后配置连接与模型。"))
        split.list_layout.addWidget(self.empty_state, 1)
    ...
    def _refresh_provider_list(self, preferred_id: str = "") -> None:
        ...
        self._refresh_model_options()
        empty = self.provider_list.count() == 0
        self.provider_list.setVisible(not empty)
    ...
    def _sync_actions(self) -> None:
        ...
        self.btn_delete_provider.setToolTip(QCoreApplication.translate('ModelsPage', '固定登录入口，可停用或退出登录') if fixed else QCoreApplication.translate('ModelsPage', '删除服务商'))
        ...
        action_label = QCoreApplication.translate('ModelsPage', '停用服务商') if enabled else QCoreApplication.translate('ModelsPage', '启用服务商')
    ...
    def _delete_provider(self) -> None:
        ...
        if item is None or item.provider.is_builtin_account:
            return
        if QMessageBox.question(self, QCoreApplication.translate('ModelsPage', '删除服务商'), QCoreApplication.translate('ModelsPage', '确定删除“{name}”吗？').format(name=item.provider.name)) != QMessageBox.StandardButton.Yes:
            return
    ...
    def get_providers(self, *, validate_connection: bool = True) -> List[Provider]:
        self._commit_active_editor_with_validation(validate_connection=validate_connection)
