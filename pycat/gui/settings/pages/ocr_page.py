...
class OcrPage(QWidget):
    ...
    def __init__(self, config: OcrConfig | None = None, *, status: Any = None,
        ...
        layout.addWidget(build_page_header("OCR", "从图片、截图和扫描 PDF 中提取文字。所有入口共用此配置。"))
        behavior = FormSection("识别方式")
        self.enabled_check = behavior.add_checkbox("启用 OCR", checked=config.enabled)
        self.backend_combo = behavior.add_combo("识别后端")
        self.backend_combo.addItem("本地 · PP-OCRv6 Small", "local")
        self.backend_combo.addItem("大模型 · 视觉识别", "vision")
        self.backend_combo.setCurrentIndex(max(0, self.backend_combo.findData(config.backend)))
        self.pdf_batch_pages_spin = behavior.add_spin("PDF 每批", value=config.pdf_batch_pages, range=(1, 32))
        self.pdf_batch_pages_spin.setSuffix(" 页")
        ...
        self.model_combo = ModelRefCombo(providers, current_model_ref=self._capability.model_target.model_ref,
                                        ...
                                        empty_label="请选择支持图片输入的模型")
        vision.form.addRow("模型", self.model_combo)
        self.prompt_edit = vision.add_text_edit("识别提示词", text=self._capability.prompt, max_height=200)
        self.prompt_edit.setMinimumHeight(130)
        reset = QPushButton("恢复默认提示词")
        ...
        runtime = FormSection("运行状态")
        self.runtime_status_label = QLabel(str(getattr(status, "detail", "") or "本地组件将在使用时检查"))
    ...
    def _sync_enabled_state(self, *_args) -> None:
        ...
        self.boundary_label.setText(
            "图片和 PDF 页面将发送给所选模型服务。按页识别，不自动切换后端；模型需支持图片输入。"
            if vision else "本地识别不上传文件；PP-OCR 组件只在识别时加载。")
    ...
    def collect(self) -> OcrConfig:
        ...
        if self.enabled_check.isChecked() and self.backend_combo.currentData() == "vision" and self.model_combo.model_ref() not in valid_models:
            raise ValueError("请为视觉 OCR 选择模型，并在模型设置中启用图片输入。")
