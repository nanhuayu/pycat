...
class StrategyPage(QWidget):
    ...
    def __init__(
        ...
        parent=None,
        *, memory_char_limit: int = 4000, user_memory_char_limit: int = 4000,
    ) -> None:
        ...
        self.memory_limit_combo.setCurrentText(str(memory_char_limit))
        self.user_memory_limit_combo.setCurrentText(str(user_memory_char_limit))
    ...
    def _setup_ui(self, agent: AgentRuntimeConfig, retry: RetryConfig, context: ContextConfig) -> None:
        ...
        context_section = FormSection("上下文压缩")
        pol = context.compression_policy
        self.tight_replay_enabled = QCheckBox("启用")
        ...
        context_section.form.addRow("工具内容压缩", tight_row)

        self.agent_auto_compress_enabled = QCheckBox("启用")
        ...
        self.comp_token_threshold_percent.setToolTip(
            "选定的完整请求达到可用输入预算的该比例后，自动生成历史摘要；原文保留。"
        )
        ...
        context_section.form.addRow("上下文压缩", compact_row)
        ...
        memory = FormSection("记忆 · 高级")
        self.memory_limit_combo = memory.add_combo("项目记忆容量（字符）", items=["4000", "8000"], current_text="4000")
        self.memory_limit_combo.setToolTip("限制各项目保存的短事实总量；不会扩大单次入模预算。降低容量保留已有内容，并允许继续精简。")
        self.user_memory_limit_combo = memory.add_combo("用户偏好容量（字符）", items=["4000", "8000"], current_text="4000")
        self.user_memory_limit_combo.setToolTip("限制跨项目用户偏好的总量，与项目记忆分别设置。降低容量保留已有内容，并允许继续精简。")
        layout.addWidget(memory.group)
