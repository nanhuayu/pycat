...
class TrayController(QObject):
    ...
    def bind_actions(self, new_action: QAction, capture_action: QAction) -> None:
        """Use the window's actions and enabled states, including settings guards."""
        new_action.triggered.connect(self.restore_window)
        if self._menu is not None:
            before = self._menu.actions()[1]
            self._menu.insertAction(before, new_action)
            self._menu.insertAction(before, capture_action)
