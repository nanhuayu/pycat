...
from PyQt6.QtCore import QEvent, QPoint, QSize, Qt, QTimer, pyqtSignal, pyqtSlot
...
class MessageActionButton(QToolButton):
    ...
    def paintEvent(self, event):
        if self.revealed or self.hasFocus() or self.underMouse():
...
class MessageWidget(QFrame):
    ...
    def _setup_ui(self):
        ...
        if self.show_header and not is_user:
            header = QHBoxLayout()
            ...
            role_label = QLabel("子助手" if self.embedded else "PyCat")
        ...
        if self.message.content or (not is_user and any(ref.mime.startswith('image/') for ref in self.delivery_refs)):
    ...
    def _add_action_buttons(self, layout):
        ...
        if (
            ...
            continue_btn = MessageActionButton()
