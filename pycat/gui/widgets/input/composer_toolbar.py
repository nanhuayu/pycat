...
class ComposerToolbar(QWidget):
    ...
    tool_approval_changed = pyqtSignal(str)
    filesystem_mode_changed = pyqtSignal(str)
    ...
    def __init__(self, parent=None):
        ...
        self._tool_approval = DEFAULT_TOOL_APPROVAL
        self._filesystem_mode = DEFAULT_FILESYSTEM_MODE
        ...
        self._send_shortcut = shortcut_sequence("send_message")
        self._setup_ui()
        ...
        for label, value in self._FILESYSTEM_MODE_ITEMS:
            ...
            action.setIcon(
                self._permission_icon(
                    Icons.UNLOCK if value == "full_access" else Icons.FOLDER,
            ...
            self._filesystem_mode_actions.addAction(action)
            self._permission_menu.addAction(action)
        self.permission_btn.setMenu(self._permission_menu)
        self._apply_access_display()
    ...
    def set_access(self, tool_approval: str, filesystem_mode: str) -> None:
        self._tool_approval = normalize_tool_approval(tool_approval)
        self._filesystem_mode = normalize_filesystem_scope_mode(filesystem_mode)
        self._apply_access_display()

    def set_tool_approval(self, approval: str) -> None:
        self.set_access(approval, self._filesystem_mode)

    def tool_approval(self) -> str:
        return self._tool_approval

    def set_filesystem_mode(self, mode: str) -> None:
        self.set_access(self._tool_approval, mode)

    def filesystem_mode(self) -> str:
        return self._filesystem_mode

    def set_access_enabled(self, enabled: bool) -> None:
        self._access_enabled = bool(enabled)
        self._sync_access_btn_enabled()
    ...
    def _apply_access_display(self) -> None:
        ...
        if self._tool_approval == "deny":
            ...
            icon_name, tone = Icons.UNLOCK, "danger"
        else:
            icon_name, tone = self._TOOL_APPROVAL_VISUALS[self._tool_approval]
        ...
        for action in self._tool_approval_actions.actions():
            action.setChecked(action.data() == self._tool_approval)
        for action in self._filesystem_mode_actions.actions():
            action.setChecked(action.data() == self._filesystem_mode)
    ...
    def _sync_access_btn_enabled(self) -> None:
        self.permission_btn.setEnabled(
            self._access_enabled and not self._context_busy
        )
    ...
    def refresh_theme(self) -> None:
        ...
        self._sync_primary_action()
        self._apply_access_display()
        for button, icon in ((self.attach_btn, Icons.PLUS), (self.session_options_btn, Icons.SLIDERS), (self.model_edit_btn, Icons.EDIT)):
            button.setIcon(Icons.get_muted(icon))
        self.set_prompt_optimize_busy(self._prompt_optimize_busy, is_streaming=self._is_streaming)
