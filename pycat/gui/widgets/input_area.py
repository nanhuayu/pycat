...
from pycat.core.commands.types import CommandAction, CommandResult, ShellInvocation
...
class InputArea(QWidget):
    ...
    def set_app_settings(self, settings: Dict[str, Any] | None) -> None:
        ...
        try:
            overrides = validate_shortcuts(self._app_settings.get("shortcuts", {}))
        except ValueError:
            overrides = {}
    ...
    def _setup_ui(self):
        ...
        self.toolbar.tool_approval_changed.connect(self.tool_approval_changed.emit)
        self.toolbar.filesystem_mode_changed.connect(self.filesystem_mode_changed.emit)
        ...
        self.toolbar.layout().removeWidget(self.context_usage_btn)
        meta = self.status_layout = QHBoxLayout()
    ...
    def resizeEvent(self, event):
        ...
        self.layout().setContentsMargins(inset, 4, inset, 8)
    ...
    def set_access(self, tool_approval: str, filesystem_mode: str) -> None:
        self.toolbar.set_access(tool_approval, filesystem_mode)

    def set_tool_approval(self, approval: str) -> None:
        self.toolbar.set_tool_approval(approval)

    def get_tool_approval(self) -> str:
        return self.toolbar.tool_approval()

    def set_filesystem_mode(self, mode: str) -> None:
        self.toolbar.set_filesystem_mode(mode)

    def get_filesystem_mode(self) -> str:
        return self.toolbar.filesystem_mode()

    def set_access_enabled(self, enabled: bool) -> None:
        self.toolbar.set_access_enabled(enabled)
    ...
    def _emit_message_payload(self, content: str) -> None:
        ...
        if mentions := self.text_input.bound_mentions():
    ...
    def append_image_edit(self, ref: str) -> None:
        """Add a verified image reference to the draft; sending remains explicit."""
        if self._submission_busy:
            return
        current = self.text_input.toPlainText()
        self.text_input.setPlainText(current + ("\n\n" if current else "") + f"请编辑图片 {ref}：")
