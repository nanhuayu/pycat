...
class ArtifactService:
    ...
    @staticmethod
    def artifact_storage_dir(*, work_dir: str, conversation_id: object = None, data_dir: str | Path | None = None) -> Path:
        session_id = str(conversation_id or "session").strip() or "session"
        return resolve_session_root(str(work_dir or "").strip(), session_id, data_dir=data_dir) / "artifact"

    @staticmethod
    def storage_base(work_dir: str, *, data_dir: str | Path | None = None) -> Path:
        raw = normalize_work_dir(work_dir)
        if WorkspaceLocation.parse(raw).is_remote:
            return resolve_project_data_root(raw, data_dir=data_dir)
        return Path(raw).expanduser().resolve() if raw else (Path(data_dir).resolve() if data_dir else Path.home().resolve())

    @staticmethod
    def artifact_file_path(*, work_dir: str, conversation_id: object = None, name: str, data_dir: str | Path | None = None) -> Path:
        safe = ArtifactService._safe_artifact_stem(name)
        return ArtifactService.artifact_storage_dir(work_dir=work_dir, conversation_id=conversation_id, data_dir=data_dir) / f"{safe}.md"

    @staticmethod
    def relative_content_path(path: Path, *, work_dir: str, data_dir: str | Path | None = None) -> str:
        root = ArtifactService.storage_base(work_dir, data_dir=data_dir)
        try:
    ...
    @staticmethod
    def resolve_content_path(content_path: str, *, work_dir: str, data_dir: str | Path | None = None) -> Path:
        path = Path(str(content_path or "").strip())
        if not path.is_absolute():
            path = ArtifactService.storage_base(work_dir, data_dir=data_dir) / path
        return path
    ...
    def write_content_file(
        ...
        *,
        ...
        artifact.content = ""
        artifact.content_path = ArtifactService.relative_content_path(target, work_dir=work_dir, data_dir=data_dir)
        artifact.content_chars = len(text)
        artifact.content_digest = hashlib.sha256(text.encode("utf-8")).hexdigest()

    @staticmethod
    def read_content_file(artifact: SessionArtifact, *, work_dir: str, data_dir: str | Path | None = None) -> str:
        if artifact.content_path:
            path = ArtifactService.resolve_content_path(artifact.content_path, work_dir=work_dir, data_dir=data_dir)
    ...
    def reconcile_artifact_files(
        ...
        *,
        ...
        for path in sorted(artifact_dir.glob("*.md"), key=lambda item: item.name.lower()):
            ...
            rel_path = ArtifactService.relative_content_path(path, work_dir=work_dir, data_dir=data_dir)
            ...
            if existing is not None and existing.content_path:
                try:
                    existing_path = ArtifactService.resolve_content_path(existing.content_path, work_dir=work_dir, data_dir=data_dir)
    ...
    def import_artifact_file(
        ...
        *,
        ...
        """
        source = ArtifactService.resolve_content_path(str(source_path or ""), work_dir=work_dir, data_dir=data_dir)
        ...
        if already_imported and existing is not None:
            current_body = strip_frontmatter(
                ArtifactService.read_content_file(imported, work_dir=work_dir, data_dir=data_dir)
            )
        ...
        if existing is None:
            ...
            current_body = strip_frontmatter(ArtifactService.read_content_file(imported, work_dir=work_dir, data_dir=data_dir))
    ...
    def _recover_imported_body(
        *,
        ...
        if not source_path or not expected_digest:
            return ""
        try:
            source = ArtifactService.resolve_content_path(source_path, work_dir=work_dir, data_dir=data_dir)
    ...
    def append_artifact(
        ...
        *,
        ...
        existing = strip_frontmatter(ArtifactService.read_content_file(artifact, work_dir=work_dir, data_dir=data_dir))
    ...
    def update_artifact_metadata(
        ...
        *,
        ...
        body = strip_frontmatter(
            ArtifactService.read_content_file(artifact, work_dir=work_dir, data_dir=data_dir)
        )
        if kind is not None:
            artifact.kind = str(kind or "").strip().lower()
        if status is not None:
