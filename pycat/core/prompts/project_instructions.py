...
from pycat.core.content.markdown import strip_frontmatter, trim_text
...
class ProjectInstructionService:
    ...
    def load(
        ...
        *,
        ...
        if workspace_service is not None and (files := workspace_service.files(work_dir)):
            if not files.stat(cls.FILENAME)["file"]:
                return []
            raw = files.read_bytes(cls.FILENAME, max_bytes=1024 * 1024).decode("utf-8", errors="replace")
            return [ProjectInstructionDocument(source=files.location.value + "/AGENTS.md",
                content=trim_text(strip_frontmatter(raw), max_chars))]
    ...
    def build_prompt_section(
        ...
        *,
        ...
        docs = cls.load(work_dir, max_chars=max_chars, workspace_service=workspace_service)
        if not docs:
            return ""
