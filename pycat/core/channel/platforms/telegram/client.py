...
class TelegramChannelClient:
    """Small Telegram Bot API client used by the channel gateway."""

    def __init__(self, *, transport=None):
        self._transport = transport

    def _http(self, config, *, timeout):
        if self._transport is not None:
            return httpx.Client(transport=self._transport, timeout=timeout)
        return self.open_http_client(config, timeout=timeout)

    def download_media(self, channel, item, directory):
        if not isinstance(item, dict) or not item.get('file_id'):
            raise ValueError('Telegram 附件标识缺失。')
        with self._http(channel.config, timeout=60.0) as client:
            response = client.post(self.build_api_url(channel, 'getFile'), json={'file_id': item['file_id']})
            response.raise_for_status()
            data = self.read_api_result(response.json(), operation='getFile')
            path = str(data.get('file_path') or '')
            if not path or ':' in path or '\\' in path or any(p in {'', '.', '..'} for p in path.split('/')):
                raise ValueError('Telegram 附件路径无效。')
            token = str(channel.config.get('bot_token') or channel.config.get('token') or '')
            base = self.normalize_api_base_url(channel.config.get('api_base_url'))
            with client.stream('GET', f'{base}/file/bot{token}/{quote(path, safe="/")}') as response:
                raw = read_media_response(response, limit=20 * 1024 * 1024)
        return save_attachment(raw, directory, name=item.get('name') or path.rsplit('/', 1)[-1],
            mime=item.get('mime', ''), image=bool(item.get('image')))

    def send_file(self, channel, *, chat_id, file, context_token='', message_thread_id=''):
        # sendDocument preserves originals (including SVG, animated and large images).
        photo = file.mime in {'image/png', 'image/jpeg'} and len(file.data) <= 10 * 1024 * 1024
        if photo:
            with Image.open(BytesIO(file.data)) as picture:
                photo = picture.width + picture.height <= 10000 and max(picture.size) / max(1, min(picture.size)) <= 20
        ...
        with self._http(channel.config, timeout=60.0) as client:
            response = client.post(self.build_api_url(channel, 'sendPhoto' if photo else 'sendDocument'),
                data=payload, files={field: (file.name, file.data, file.mime)})
            response.raise_for_status()
            self.read_api_result(response.json(), operation='send attachment')
    ...
    def get_updates(
        ...
        *,
        ...
        with self._http(config, timeout=max(10.0, float(poll_timeout) + 10.0)) as client:
    ...
    def send_text_message(
        ...
        *,
        ...
        with self._http(config, timeout=20.0) as client:
