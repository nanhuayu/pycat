...
def _build_message_envelope(
    ...
    *,
    ...
    return TelegramUpdateEnvelope(
        ...
        event_type=event_type,
        meta=meta,
        media=[] if event_type == 'callback_query' else extract_telegram_media(message),
    )


def extract_telegram_media(message):
    photos = [item for item in message.get('photo', []) if isinstance(item, dict) and item.get('file_id')]
    if photos:
        photo = max(photos, key=lambda p: int(p.get('width') or 0) * int(p.get('height') or 0))
        return [{'file_id': photo['file_id'], 'image': True, 'name': ''}]
    for kind in ('document', 'voice', 'audio', 'video', 'animation', 'sticker'):
        item = message.get(kind)
        if isinstance(item, dict) and item.get('file_id'):
            return [{'file_id': item['file_id'], 'name': str(item.get('file_name') or ''),
                     'mime': str(item.get('mime_type') or ''), 'image': False}]
    return []
