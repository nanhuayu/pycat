...
def normalize_feishu_webhook_payload(
    ...
    *,
    ...
    media = extract_feishu_media(message_type, message.get('content'), message_id)
...
def extract_feishu_media(kind, raw_content, message_id):
    parsed = _parse_content(raw_content)
    parts = []
    if kind == 'post':
        post = parsed.get('zh_cn') or parsed.get('en_us') or parsed
        parts = [item for row in post.get('content', []) if isinstance(row, list)
                 for item in row if isinstance(item, dict) and item.get('tag') == 'img']
    elif kind in {'image', 'file', 'audio', 'media'}:
        parts = [parsed]
    result = []
    for item in parts:
        key = item.get('file_key') or item.get('image_key')
        if key:
            result.append({'message_id': message_id, 'key': str(key),
                'type': 'image' if kind in {'image', 'post'} else 'file',
