...
from pycat.core.channel.envelope import message_from_channel
...
class ChannelQueue:
    ...
    def enqueue(self, source: str, content: str, meta: dict[str, Any] | None = None, *, media=None) -> Message:
        message = message_from_channel(source, content, meta)
        if media:
            # Platform transport data is removed before the message enters a conversation.
            message.metadata['_channel_media'] = media
        with self._lock:
