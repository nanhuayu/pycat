...
from pycat.models.model_ref import provider_matches_name, split_model_ref
...
def select_default_provider_model(
    ...
    *,
    ...
    if (
        ...
        and resolved_default.provider.effective_model_profile(resolved_default.model).model_type == "chat"
    ):
        return ResolvedModelSelection(
    ...
    for provider in provider_list:
        models = tuple(model for model in provider_model_ids(provider) if provider.effective_model_profile(model).model_type == "chat")
        if models:
...
def resolve_model_target(
    ...
    *,
    ...
    if not primary_model_id and primary_provider is not None:
        primary_ids = tuple(model for model in provider_model_ids(primary_provider) if primary_provider.effective_model_profile(model).model_type == "chat")
