...
def build_skill_prompt_section(
    ...
    *,
    ...
    skill_manager = SkillsManager(work_dir, data_dir=getattr(conversation, "data_dir", None))
    ...
    if pycat_assistant_enabled:
        for skill in skill_manager.list_skills():
            ...
            if (
                (spec.user_invocable or not spec.disable_model_invocation)
                and not scan_for_threats(str(skill.content or ""), scope="strict")
            ):
    ...
    if loaded_skill is not None:
        ...
        threats = scan_for_threats(str(loaded_skill.content or ""), scope="strict")
        if threats:
            runtime_lines.append("status: blocked")
            runtime_lines.append(f"blocked_reasons: {', '.join(threats)}")
        else:
