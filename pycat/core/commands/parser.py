...
def inside_code(text: str, position: int) -> bool:
    """Ignore fenced and inline code while offering input actions."""
    fence = None
    inline = False
        ...
        if stripped.startswith(('```', '~~~')):
            ...
            for index, char in enumerate(line):
                if char == '`' and (index == 0 or line[index - 1] != '\\'):
                    inline = not inline
    return fence is not None or inline
...
def extract_command_query(
    ...
    *,
    ...
    for trigger in prefixes:
        ...
        if idx == -1:
            continue
        if inside_code(text, line_start + idx):
            continue
