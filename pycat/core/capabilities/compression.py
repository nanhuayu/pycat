...
from pycat.core.capabilities import CapabilityConfig
...
class CapabilityCompressor:
    ...
    async def _run_capability(
        ...
        *,
        ...
        # unusable, so re-ask with a reinforced single-key instruction.
        if result.error not in {"model_output_not_json", "model_output_invalid_schema"}:
            return result
        logger.warning(
            "Capability compression output rejected (%s); retrying with reinforced contract",
            result.error,
        )
        retried = await self._invoke_capability(
            capability,
            f"{user_prompt}\n\nIMPORTANT: Respond with exactly one JSON object containing only the key \"summary\".",
            conversation=conversation,
            selection=selection,
            images=images,
            purpose=purpose,
