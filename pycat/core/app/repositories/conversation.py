...
class ConversationRepository:
    ...
    def _path(self, conversation_id: str) -> Path:
        if not isinstance(conversation_id, str) or not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_-]{0,127}", conversation_id):
    ...
    def load(self, conversation_id: str) -> Conversation | None:
        try:
            ...
            if file_path.exists():
                conversation = Conversation.from_json(file_path.read_text(encoding="utf-8"))
                conversation.data_dir = str(self.data_dir)
                if not conversation.work_dir:
                    for artifact in conversation.get_state().artifacts.values():
                        if artifact.content_path.startswith(".pycat/"):
                            artifact.content_path = artifact.content_path[len(".pycat/"):]
                return conversation
        except Exception as exc:
