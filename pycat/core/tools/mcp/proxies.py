...
from pycat.core.tools.mcp.naming import build_mcp_tool_name
...
class McpProxyTool(BaseTool):
    ...
    async def execute(self, arguments: Dict[str, Any], context: ToolContext) -> ToolResult:
        try:
            ...
            if isinstance(result.content, list):
                result.content = [
                    {**block, 'text': self._normalize_result_paths(str(block.get('text') or ''), context=context, conversation_id=conversation_id)}
                    if isinstance(block, dict) and block.get('type') == 'text' else block
                    for block in result.content
                ]
            else:
                result.content = self._normalize_result_paths(
                    result.to_string(), context=context, conversation_id=conversation_id,
                )
            return result
        except Exception as exc:
    ...
    def _session_tool_results_dir(self, *, context: ToolContext, conversation_id: object = None) -> Path | None:
        ...
        try:
            ...
            return None
        return resolve_session_root(root, session_id, data_dir=context.data_dir) / "tool-call" / "mcp-output"
    ...
    def _normalize_result_paths(self, text: str, *, context: ToolContext, conversation_id: object = None) -> str:
        ...
        try:
            work_root = Path(raw_work_dir).expanduser().resolve() if raw_work_dir and not raw_work_dir.startswith("ssh://") else None
        except Exception:
            work_root = None
