...
class WriteToFileTool(BaseTool):
    ...
    async def execute(self, arguments: Dict[str, Any], context: ToolContext) -> ToolResult:
        return await context.run_file_io(self._execute, arguments, context)

    def _execute(self, arguments: Dict[str, Any], context: ToolContext) -> ToolResult:
        path_text = str(arguments.get("path") or "").strip()
        if not path_text:
            return ToolResult("path is required.", is_error=True)
        try:
            path = context.resolve_workspace_path(path_text)
            before_digest = context.file_digest(path)
            context.write_bytes(path, str(arguments.get("content") or "").encode("utf-8"), expected=before_digest)
            ...
            return ToolResult(
                ...
                metadata=file_change_metadata(
                    ...
                    after_digest=context.file_digest(path),
                    summary=summary,
                ),
            )
...
class EditFileTool(BaseTool):
    ...
    def _execute(self, arguments: Dict[str, Any], context: ToolContext) -> ToolResult:
        ...
        if not context.file_is_file(path):
            return ToolResult(f"File not found: {path_text}", is_error=True)
        try:
            original = context.read_bytes(path)
            ...
            context.write_bytes(path, updated, expected=before_digest)
...
class DeleteFileTool(BaseTool):
    ...
    def _execute(self, arguments: Dict[str, Any], context: ToolContext) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(str(exc), is_error=True)
        if not (context.files.stat(path)["exists"] if context.files else path.exists()):
            return ToolResult(f"Path not found: {path_text}", is_error=True)
        try:
            before_digest = context.file_digest(path)
            context.check_file_cancelled()
            if context.files:
                context.files.call("delete", path, write=True, recursive=bool(arguments.get("recursive")))
            elif path.is_dir():
