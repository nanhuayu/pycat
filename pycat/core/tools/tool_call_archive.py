...
class ToolCallArchiveService:
    ...
    @staticmethod
    def _error(tool_name: str, text: str, metadata: dict[str, Any]) -> ToolCallArchiveResult:
        ...
        lines.append(f"message={str(text or '').strip() or code}")
        display = "\n".join(lines)
        summary_text = next((line.strip() for line in str(text or "").splitlines() if line.strip()), code)
        return ToolCallArchiveResult(
            display=display,
            total_chars=len(str(text or "")),
            strategy="error",
            summary=f"{tool_name or 'tool'} failed: {summary_text}"[:220],
