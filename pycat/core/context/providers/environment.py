...
from pycat.core.context.sections import build_environment_info, build_workspace_info
...
class EnvironmentProvider(MessageProviderMixin):
    ...
    def build_items(self, context: ProviderContext):
        ...
        if bool(getattr(prompt_cfg, "include_environment", True)):
            blocks.append(build_environment_info(
                cwd=context.work_dir, include_time=False,
                shell_config=getattr(context.app_config, "shell", None),
            ))
