...
_SENSITIVE_KEY_RE = re.compile(
    r"(authorization|api[_-]?key|apikey|access[_-]?token|refresh[_-]?token|auth[_-]?token|secret|cookie|password|proxy|x-(?:user|enterprise|tenant)-id)",
    re.IGNORECASE,
)
...
def resolve_debug_trace_dir(*, conversation_id: str, work_dir: str = ".", data_dir: str | Path | None = None) -> Path:
    ...
    return resolve_session_root(root, session_id, data_dir=data_dir) / "debug"
...
def finish_run_debug_trace(
    ...
    *,
    ...
    execution_finished: bool | None = None,
) -> None:
    ...
    debug_trace.record_event(
        ...
        data={key: value for key, value in {"result_saved": result_saved, "execution_finished": execution_finished}.items() if value is not None},
    )
