...
def tool_call_kind(name: str) -> str:
    """Classify a tool name for display and tree grouping."""
    text = str(name or '')
    if text == 'agent__run':
        return 'subagent'
...
class Conversation:
    ...
    # Bound by the application repository; never accepted from persisted input.
    data_dir: str = field(default="", repr=False, compare=False)
    ...
    def clone(self) -> "Conversation":
        result = Conversation.from_dict(self.to_dict())
        result.data_dir = self.data_dir
        return result

    def __post_init__(self) -> None:
        self.work_dir = normalize_work_dir(self.work_dir)
        if not isinstance(self.settings, dict):
            self.settings = {}
        self.settings.setdefault("tool_approval", DEFAULT_TOOL_APPROVAL)
        self.settings.setdefault("filesystem_mode", DEFAULT_FILESYSTEM_MODE)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'work_dir': normalize_work_dir(self.work_dir),
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Conversation':
        ...
        # Missing fields in saved sessions meant default/confined. Materialize
        # that policy before construction so new-session defaults do not widen it.
        legacy_permission = str(settings.pop('permission_preset', '') or '').strip().lower()
        if 'tool_approval' not in settings:
            settings['tool_approval'] = (
                'allow'
                if legacy_permission in {'allow', 'full_access'}
                else normalize_tool_approval(legacy_permission)
            )
        else:
            settings['tool_approval'] = normalize_tool_approval(settings.get('tool_approval'))
        if 'filesystem_mode' not in settings:
            settings['filesystem_mode'] = (
                'full_access'
                if legacy_permission in {'allow', 'full_access'}
                else 'confined'
            )
