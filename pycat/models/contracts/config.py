...
class CompressionPolicyConfig:
    tight_replay_enabled: bool = True
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "tight_replay_enabled": bool(self.tight_replay_enabled),
...
class RetryConfig:
    ...
    def from_dict(data: Mapping[str, Any] | None) -> "RetryConfig":
        d = _as_dict(dict(data) if data is not None else {})
        return RetryConfig(
            max_retries=int(_as_float(d.get("max_retries"), 5)),
            base_delay=float(_as_float(d.get("base_delay"), 5.0)),
...
class ShellConfig:
    ...
    def from_dict(data: Mapping[str, Any] | None) -> "ShellConfig":
        d = _as_dict(dict(data) if data is not None else {})
        backend = _as_str(d.get("backend"), "auto").strip().lower() or "auto"
        if backend not in {"auto", "cmd", "powershell", "wsl", "bash", "zsh", "sh", "fish", "custom"}:
            backend = "auto"
        executable = _as_str(d.get("executable", d.get(f"{backend}_executable")), "").strip()

        encoding = _as_str(d.get("output_encoding"), "auto").strip().lower() or "auto"
        if encoding not in {"auto", "utf-8", "utf-16", "system", "gb18030"}:
            encoding = "auto"
        ...
        return ShellConfig(
            ...
            arguments=tuple(str(arg) for arg in d.get("arguments", []) if isinstance(arg, str))
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "wait_seconds": _clamp_int(int(self.wait_seconds or 10), 5, 600),
        }
...
class OcrConfig:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "pdf_batch_pages": _clamp_int(int(self.pdf_batch_pages or 8), 1, 32),
            "backend": self.backend,
        }
...
class AppConfig:
    ...
    def from_dict(data: Mapping[str, Any] | None) -> "AppConfig":
        ...
        return AppConfig(
            ...
            ocr=OcrConfig.from_dict(_as_dict(d.get("ocr"))),
        )
