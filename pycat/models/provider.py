...
from pycat.models.image_api import CODEX_IMAGES, ImageAPI
from pycat.models.model_profile import DEFAULT_REASONING_OPTIONS, ModelProfile, recommended_reasoning_codec
from pycat.models.model_ref import (
    build_model_ref,
...
PROVIDER_SCHEMA_VERSION = 10
WORKBUDDY_ORIGIN = 'https://www.workbuddy.cn'
ACCOUNT_AUTH_LABELS = {'chatgpt': 'ChatGPT / Codex', 'workbuddy': 'WorkBuddy / CodeBuddy'}
...
class Provider:
    ...
    image_api: ImageAPI = field(default_factory=ImageAPI)
    ...
    def normalize_inplace(self) -> None:
        ...
        self.auth_type = self.catalog_key if self.is_builtin_account else self.auth_type
        if self.auth_type not in ACCOUNT_AUTH_LABELS:
            self.auth_type = 'api_key'
        if self.auth_type == 'chatgpt':
            self.api_type = OPENAI_RESPONSES
            self.api_base = 'https://chatgpt.com/backend-api/codex'
            self.api_key = ''
        elif self.auth_type == 'workbuddy':
            self.api_type = OPENAI_COMPATIBLE
            self.api_base = WORKBUDDY_ORIGIN + '/v2'
            self.api_key = ''
        ...
        if not isinstance(self.image_api, ImageAPI):
            self.image_api = ImageAPI.from_dict(self.image_api)
        if self.auth_type == 'chatgpt':
            self.image_api = ImageAPI(protocol=CODEX_IMAGES)
        elif self.image_api.protocol == CODEX_IMAGES:
            self.image_api = ImageAPI()

    @property
    def supports_image_api(self) -> bool:
        return self.auth_type in {'api_key', 'chatgpt'}

    @property
    def effective_image_api(self) -> ImageAPI:
        """Account credentials may only be sent to the fixed account endpoint."""
        return ImageAPI(protocol=CODEX_IMAGES) if self.auth_type == 'chatgpt' else self.image_api
    ...
    @property
    def is_builtin_chatgpt(self) -> bool:
        return self.catalog_key == 'chatgpt'

    @property
    def is_builtin_account(self) -> bool:
        return self.catalog_key in ACCOUNT_AUTH_LABELS

    @property
    def account_label(self) -> str:
        return ACCOUNT_AUTH_LABELS.get(self.auth_type, self.name)
    ...
    def requires_api_key(self) -> bool:
        return self.auth_type not in ACCOUNT_AUTH_LABELS and self.api_type != OLLAMA_CHAT
    ...
    def model_profile_template(self) -> ModelProfile:
        """Return an unsaved editor template; discovery keeps explicit metadata."""
        codec = recommended_reasoning_codec(
            self.api_type, "openrouter" if self.is_openrouter_route else self.catalog_key,
        )
        return ModelProfile(reasoning_codec=codec,
                            reasoning_options=list(DEFAULT_REASONING_OPTIONS.get(codec, ("inherit",))))
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'image_api': self.image_api.to_dict(),
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Provider':
        ...
        return cls(
            ...
            image_api=ImageAPI.from_provider_data(data),
            models=raw_models,
    ...
    def get_headers(self, model_id: str = "") -> Dict[str, str]:
        ...
        image_request = bool(model_id and self.effective_model_profile(model_id).model_type == 'image')
        if self.is_anthropic_native and not image_request:
            if self.api_key:
    ...
    def get_chat_endpoint(self) -> str:
        ...
        if self.auth_type == 'workbuddy':
            return WORKBUDDY_ORIGIN + '/v2/chat/completions'
