...
[项目介绍](../../README.md) · [桌面使用指南](user-guide_zh.md) · [English](developers.md)
...
PyCat 以文件保存主要业务内容，让编辑器、脚本和其他工具也能检查与利用这些资料：

| 内容 | 存储与用途 |
| --- | --- |
| 配置和会话 | 应用数据目录中的 JSON；包含模型配置、会话与已保存状态 |
| 会话输入和归档 | 本地项目的 `.pycat/sessions/<会话 ID>/`；保存输入快照、工具原文、图片和相关元数据 |
| 运行证据 | 会话目录下 `debug/` 的 JSONL 事件和可选诊断附件；运行检查与 SDK 使用同一读取入口 |
| 项目知识与记忆 | Markdown 文件；可以阅读、搜索和备份 |
| 交付成果 | 项目中的普通文件；可用现有编辑器继续加工，也可按需要纳入版本管理 |

原文归档与压缩摘要分开保存，摘要可以重新生成。默认诊断只记录轻量事件；开启详细日志后保存脱敏请求、响应和流式附件。记录反映实际捕获范围，不保证外部系统可以重放，也不保存模型未返回的内部推理。

“以文件保存”不意味着所有状态都是纯文本：搜索索引和部分后台作业账本使用 SQLite，活动进程和网络连接则属于内存中的运行状态。源码使用者可指定 `data_dir`；没有本地项目或使用 SSH 工作区时，会话资料由应用数据目录管理。备份时应同时覆盖应用数据目录和项目内的 `.pycat`，不能只复制交付文件；应先关闭对应应用。不要把整个 `.pycat` 直接提交到公开仓库，也不要在运行中手动修改内部状态文件。

文件与结构化结果便于接入外部分析程序。SDK 提供运行轨迹和节点明细读取；CLI 的 JSON / 逐行事件可接入自己的评估工作流。详细接口与参数以公共接口签名和 `--help` 为准。
