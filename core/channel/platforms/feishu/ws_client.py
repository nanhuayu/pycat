...
from core.channel.async_connection import AsyncConnectionHandle, run_async_connection_thread
...
from core.channel.platforms.feishu.router import normalize_feishu_webhook_payload
from core.channel.platforms.feishu.ws_protocol import (
    GEN_ENDPOINT_URI,
    HEADER_BIZ_RT,
    HEADER_MESSAGE_ID,
    HEADER_SEQ,
...
@dataclass
class FeishuWebSocketClientHandle(AsyncConnectionHandle):
    partial_payloads: dict[str, list[bytes]] = field(default_factory=dict)


class _FeishuWebSocketRunner:
    def __init__(self, context: ChannelHost, channel: ChannelConfig, handle: FeishuWebSocketClientHandle) -> None:
...
def start_feishu_ws_client(
...
) -> FeishuWebSocketClientHandle | None:
    ...
    handle = FeishuWebSocketClientHandle(channel_id=channel_id)
    runner = _FeishuWebSocketRunner(context, channel, handle)
    run_async_connection_thread(
        handle,
        thread_name=f"PyCat-FeishuWS-{channel_id}",
        run=runner.run,
    )
    logger.info("Started Feishu websocket worker for channel %s", channel_id)
    return handle

__all__ = ["FeishuWebSocketClientHandle", "start_feishu_ws_client"]
